module codecat_addr::codecat {
  ...
  public entry fun add_code(account: &signer, name: String, uri: String) acquires CodeList {
		let signer_address = signer::address_of(account);
  ...
  public entry fun test_flow(admin: signer) acquires CodeList {
    ...
    add_code(&admin, string::utf8(b"github"), string::utf8(b"https://github.com/jjwygjj/aptos-codecat.git"));
    ...
    assert!(code_record.address == signer::address_of(&admin), 6);

    remove_code(&admin, string::utf8(b"github"));
    let code_list = borrow_global<CodeList>(signer::address_of(&admin));
    assert!(!table::contains(&code_list.codes, string::utf8(b"github")), 7);
  }
}